...
from tensorflow.keras import utils,layers,initializers,activations
...
class PositionEmbedding(layers.Layer):
    ...
    Arguments:
        ...
        initializer: The initializer to use for the embedding weights. Default to
          "glorot_uniform".
    """

    def __init__(self,max_length,initializer="glorot_uniform",**kwargs):

        super().__init__(**kwargs)
        if max_length is None:
            raise ValueError("`max_length` must be an Integer, not `None`.")
        self._max_length=max_length
        self._initializer=initializer.get(initializer)
